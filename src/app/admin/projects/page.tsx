...
import { Plus, Search, Edit3, Trash2, Eye, FileText, Sparkles, Loader2, X, Save, BarChart3 } from "lucide-react"
...
export default function AdminProjectsPage() {
  ...
  useEffect(() => { fetchProjects() }, [filter, statusFilter, search])

  const fetchProjects = async () => {
    const supabase = createClient()
    let query = supabase.from("projects").select("*").order("created_at", { ascending: false })
    if (filter !== "全部") query = query.eq("category", filter)
  ...
  const stats = {
    ...
    premium: projects.filter(p => p.is_premium).length,
  ...
  const batchPublish = async () => {
    ...
    const supabase = createClient()
    await supabase.from("projects").update({ status: "published" }).in("id", Array.from(selectedIds))
    setSelectedIds(new Set())
    fetchProjects()
  }
  ...
  const openEditor = (proj?: Project) => {
    if (proj) {
      ...
      setFormPremium(false); setFormFeatured(false); setFormPractitioner(false)
  ...
  const handleSave = async () => {
    if (!formTitle.trim()) { alert("请填写项目标题"); return }
    setSaving(true)
    const supabase = createClient()
    ...
    const projectData = {
      ...
      is_premium: formPremium, is_featured: formFeatured,
      is_practitioner_recommended: formPractitioner, recommend_reason: formRecommendReason,
      status: editingId ? (projects.find(p => p.id === editingId)?.status || "draft") : "draft",
      author_id: user?.id || null, updated_at: new Date().toISOString(),
    }
    let error
    if (editingId) {
      const { error: e } = await supabase.from("projects").update(projectData).eq("id", editingId)
      error = e
    } else {
      const { error: e } = await supabase.from("projects").insert({ ...projectData })
      error = e
    }
    setSaving(false)
    if (error) { alert("保存失败：" + error.message) }
    else { closeEditor(); fetchProjects(); router.refresh() }
  }
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">管理创业项目的内容发布与数据分析</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {([
          { label: "全部项目", value: stats.total, icon: FileText, textColor: "text-blue-600" },
          { label: "已发布", value: stats.published, icon: Eye, textColor: "text-green-600" },
          { label: "草稿", value: stats.draft, icon: Edit3, textColor: "text-amber-600" },
          { label: "付费内容", value: stats.premium, icon: Sparkles, textColor: "text-purple-600" },
          { label: "官方推荐", value: stats.featured, icon: BarChart3, textColor: "text-rose-600" },
        ] as const).map((s, i) => (
          <Card key={i} className="hover:shadow-sm transition-shadow">
            <CardContent className="p-4">
              <div className={`flex items-center gap-2 ${s.textColor} mb-1`}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* 批量操作 */}
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-2 ml-auto bg-primary/5 rounded-lg px-3 py-1.5 border border-primary/20">
            <span className="text-sm font-medium text-primary">已选 {selectedIds.size} 项</span>
            <Button size="sm" variant="outline" className="h-7 text-xs" onClick={batchPublish}>批量发布</Button>
            <Button size="sm" variant="outline" className="h-7 text-xs text-red-500 hover:text-red-600" onClick={batchDelete}>批量删除</Button>
            <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setSelectedIds(new Set())}>取消</Button>
          </div>
        )}
      </div>
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">共 {projects.length} 个项目</CardTitle>
            <label className="flex items-center gap-2 text-xs text-muted-foreground cursor-pointer select-none">
              <input type="checkbox" checked={selectedIds.size === projects.length && projects.length > 0} onChange={toggleSelectAll} className="rounded" />
              全选
            </label>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-12"><p className="text-muted-foreground">暂无项目，点击上方按钮创建第一个</p></div>
          ) : (
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {proj.is_practitioner_recommended && <Badge variant="outline" className="text-[10px] text-green-600 border-green-300 bg-green-50">👥实践者</Badge>}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                        <Badge variant="secondary" className="text-[10px]">{proj.category}</Badge>
                        <span>{proj.difficulty}</span>
                        <span>·</span>
                        <span>{new Date(proj.created_at).toLocaleDateString("zh-CN")}</span>
                        <span>·</span>
                        <span className="truncate">{proj.hook?.slice(0, 25) || ""}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 ml-3">
                    <Badge variant={proj.status === "published" ? "success" : "warning"} className="text-[10px]">
                      {proj.status === "published" ? "已发布" : "草稿"}
                    </Badge>
                    <Button variant="ghost" size="sm" onClick={() => handlePublish(proj.id, proj.status)} title={proj.status === "published" ? "撤销发布" : "发布"}>
                      {proj.status === "published" ? "撤销" : "发布"}
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => openEditor(proj)} title="编辑"><Edit3 className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" className="text-red-400 hover:text-red-600" title="删除" onClick={() => handleDelete(proj.id)}>
        ...
        <div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/50 py-8 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b sticky top-0 bg-white rounded-t-xl z-10">
              <h2 className="text-lg font-bold">{editingId ? "编辑项目" : "新建项目"}</h2>
            ...
            <div className="p-6 max-h-[calc(100vh-200px)] overflow-y-auto">
              {!previewMode && (
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="col-span-2">
                    ...
                    <Input value={formTitle} onChange={(e) => setFormTitle(e.target.value)} placeholder="例如：闲鱼AI代写服务" />
                  </div>
                  <div>
                    ...
                    <select value={formCategory} onChange={(e) => setFormCategory(e.target.value)} className="w-full h-9 rounded-md border px-3 text-sm bg-background">
                      {["闲鱼", "小红书", "AI工具", "短视频", "电商", "更多"].map((c) => (<option key={c} value={c}>{c}</option>))}
                    </select>
                  </div>
                  <div>
                  ...
                  <div className="col-span-2 grid grid-cols-2 gap-4 p-4 bg-amber-50 rounded-lg border border-amber-200">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <input type="checkbox" id="practitioner" checked={formPractitioner} onChange={(e) => setFormPractitioner(e.target.checked)} className="rounded" />
                    ...
                    <div className="col-span-2">
                      ...
                      <Input value={formRecommendReason} onChange={(e) => setFormRecommendReason(e.target.value)} placeholder="为什么推荐这个项目？" className="text-sm" />
                    </div>
                  </div>
                </div>
              )}
              <div>
                ...
                {previewMode ? (
                  ...
                  <Textarea value={formContent} onChange={(e) => setFormContent(e.target.value)} className="font-mono text-sm min-h-[400px]" placeholder="使用 Markdown 编写项目内容..." />
                )}
              </div>
            </div>
          </div>
